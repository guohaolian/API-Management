...
- [BACKEND-NEXUS](BACKEND-NEXUS/)：后端（Python + Robyn）
- [FRONTEND-NEXUS](FRONTEND-NEXUS/)：前端（React + Vite）
- [nexus-api-codegen](nexus-api-codegen/)：前端代码生成器（Node CLI，命令 `nexus`）

## 快速开始（本机开发）

### 1) 启动后端（BACKEND-NEXUS）

**依赖**：Python（见 [BACKEND-NEXUS/pyproject.toml](BACKEND-NEXUS/pyproject.toml) 里的 `requires-python`）、`uv`、PostgreSQL（以及按需的 Redis）。
...
2. 准备环境变量（复制 `BACKEND-NEXUS/.env.example` 为 `BACKEND-NEXUS/.env`，字段说明见 [BACKEND-NEXUS/README.md](BACKEND-NEXUS/README.md)）。
  - 默认端口：`PORT=1024`
...
3. 初始化数据库（首次搭建）：
  ...
  3) 在 [BACKEND-NEXUS/.env.example](BACKEND-NEXUS/.env.example) 的基础上填写 `DATABASE_*` 与 `DATABASE_URI`
...
4. 数据库迁移（模型变更时）：
  ...
  目前仓库提供了迁移脚本（见 [BACKEND-NEXUS/database/db-migrate.ps1](BACKEND-NEXUS/database/db-migrate.ps1)），用于在你修改 `database/models.py` 后生成/执行迁移。
...
也可以在仓库根目录直接执行 [run-dev.ps1](run-dev.ps1)，一次同时启动前后端。
...
前端开发环境变量已在 [FRONTEND-NEXUS/.env.development](FRONTEND-NEXUS/.env.development) 中给出：
...
#### 让 `nexus` 命令全局可用（推荐本机用）

在 [nexus-api-codegen](nexus-api-codegen/) 目录执行：
...
当前生成器请求后端的 baseURL 在 [nexus-api-codegen/src/request/index.ts](nexus-api-codegen/src/request/index.ts) 中配置；本仓库默认按本机后端 `http://127.0.0.1:1024` 使用。
