## 用户通过命令行操作 nexus 是怎么实现的？

用户通过命令行操作 `nexus` 的实现机制主要依赖于 `package.json` 中的 `bin` 字段配置以及 `src/cli/index.ts` 中的逻辑实现。

以下是详细的实现原理解析：

**1. 命令行入口配置 (`package.json`)**

在 [package.json](./package.json) 中，我们配置了 `bin` 字段：
...
-   **作用**：这告诉 npm（或 yarn/pnpm），当用户安装这个包时，需要将 `nexus` 这个命令注册到系统的 PATH 中。
-   **映射**：当用户在终端输入 `nexus` 时，实际执行的是 `dist/cli/index.js` 这个文件。

**2. 可执行脚本声明 (`src/cli.ts`)**

在 [src/cli/index.ts](./src/cli/index.ts) 的第一行，我们添加了 Shebang：
...
-   **初始化程序**：
    ...
    program.name("nexus").description("...").version("0.0.1");
    ```

    这定义了主命令 `nexus` 的基本信息。

-   **注册 `init` 子命令**：
    在 [src/cli/index.ts](./src/cli/index.ts) 中，我们注册了 `init` 命令：
    ...
    program
        ...
        .action(() => {
            // ... 生成 nexus.config.json 的逻辑
        });
    ```

    -   `.command('init')`：监听 `nexus init` 命令。
    -   `.action(...)`：定义了当用户执行该命令时要运行的具体函数。在这里，我们编写了创建 `nexus.config.json` 文件的逻辑。
...
当用户执行 `nexus init` 时，系统发生了以下交互：

1.  操作系统查找 PATH 中的 `nexus` 命令。
2.  找到 `nexus` 指向的 `dist/cli/index.js` 文件。
3.  通过 `node` 执行该文件。
...
确保您的功能开发完成，并且在本地测试通过（比如您刚才做的 `nexus init` 测试）。

### 2. 自动化构建配置 (已帮您优化)

为了防止“忘记编译就发布”的情况，我已经帮您修改了 [package.json](./package.json) 增加了两个关键配置：
...
2.  **执行命令**：
    ...
    # 或者去其他目录测试
    cd ../some-test-dir
    node /path/to/your/project/dist/cli/index.js init
    ```
...
`npm link` 会把您的本地包“软链接”到全局 npm 目录中。这样您就可以像已经安装了该包一样，在终端的任何位置直接使用 `nexus` 命令。
...
2.  **创建链接**（在项目根目录下执行）：
    ...
    _这会读取 `package.json` 中的 `bin` 配置，把 `nexus` 命令注册到系统全局。_

3.  **验证使用**（在任意目录）：

    ```bash
    mkdir ~/Desktop/test-nexus
    cd ~/Desktop/test-nexus
    nexus init  # 直接使用 nexus 命令！
    ```
...
## nexus 登录过程：在 cli 中输入用户名和密码，发请求返回一个 token。token 保存在哪里？每次进行 cli 操作时怎么鉴权？
...
Token 会保存在用户主目录下的 `.nexusrc` 文件中。这是一个隐藏文件，类似于 `.npmrc` 或 `.gitconfig`。

### 2. 核心组件实现

-   **[TokenManager](./src/utils/data-manager.ts)**：专门负责读写 Token。
    -   使用 `os.homedir()` 获取用户主目录。
    -   写入时设置权限为 `0o600`（仅当前用户可读写），提高安全性。
-   **[nexus login 命令](./src/cli/user.ts)**：
...
async function fetchData() {
    ...
    if (!token) {
        console.error("请先运行 nexus login 进行登录");
        return;
    }
