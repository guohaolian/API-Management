...
const readConfig = () => {
    if (!fs.existsSync(CONFIG_FILE_NAME)) {
        console.warn("You need to run `nexus init` first");
        process.exit(1);
    }
    try {
        ...
        console.warn(
            "Failed to read config file, please delete it and run `nexus init` again"
        );
        process.exit(1);
    }
};

// 拉取service信息并存储到nexus.config.json
...
// 入口：基于配置文件中存储的service信息，拉取全部service的全部api
export const pullAllApisInAllServices = async () => {
    const config = readConfig();
    if (!config.services || Object.keys(config.services).length === 0) {
        console.warn(
            "No service has been added yet. Please use 'nexus add <name:uuid@version>' to add a service first."
        );
        process.exit(1);
    }
    // 清空输出目录下的所有文件
    ...
    // 遍历配置文件中存储的service信息，拉取全部service的全部api
    for (const [name, UuidAndVersion] of Object.entries(config.services)) {
        ...
        if (!service_uuid || !version) {
            console.warn(
                `Invalid service info ${UuidAndVersion} for service ${name}. Please use 'nexus add <name:uuid@version>' to add instead.`
            );
            continue;
        }
